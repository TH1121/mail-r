      ...
      <el-dialog class="resend-table" v-model="showResendList" :title="$t('resendTokenList')">
        <el-table :data="resendList">
          ...
          <el-table-column :width="tokenColumnWidth" property="value" label="Token"
                           :show-overflow-tooltip="true"/>
          <el-table-column :width="70" :label="$t('delete')" fixed="right" align="center">
            <template #default="{ row }">
              <el-button class="opt-button" size="small" type="danger" :loading="settingLoading"
                         @click="delResendToken(row.key)">
                <Icon icon="material-symbols:delete-outline-rounded" width="16" height="16"/>
              </el-button>
            </template>
          </el-table-column>
...
function getSettings() {
  ...
  settingQuery().then(settingData => {
    ...
    loginVerifyCount.value = setting.value.loginVerifyCount ?? 1
    resetNoticeForm()
    resetAddS3Form()
    resetEmailPrefix()
    resetBlackList()
...
function delResendToken(domain) {
  ElMessageBox.confirm(t('delConfirm', {msg: domain}), {
    confirmButtonText: t('confirm'),
    cancelButtonText: t('cancel'),
    type: 'warning'
  }).then(() => {
    editSetting({resendTokens: {[domain]: ''}})
  })
}
